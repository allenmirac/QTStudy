...
void DoTcpServer::incomingConnection(qintptr socketDescriptor)
{
...
void DoTcpServer::userExitHandle_Slots(int socketDescriptor)
{
	...
	QSqlQuery q;
    q.exec(QString("SELECT id,descriptor,account,name FROM userinfo WHERE descriptor = '%1'").arg(socketDescriptor));
	while (q.next())
	{
        int id = q.value(0).toInt();
        int descriptorTemp = q.value(1).toInt();
        QString userAccount = q.value(2).toString();
        QString userName = q.value(3).toString();
		//判断从数据库查询出来的套接字描述符是否与请求客户端的套接字相同
		if (descriptorTemp == socketDescriptor)
		{
            //修改数据库的登录状态，可能会出现数据库在安全更新模式下where条件没有主键无法更新的情况
            //https://blog.csdn.net/weixin_48419914/article/details/121685679
            q.exec(QString("UPDATE userinfo SET status = '0' WHERE id = '%1'").arg(id));

			//删除用户的ip地址
            q.exec(QString("UPDATE userinfo SET ip = '0.0.0.0' WHERE id = '%1'").arg(id));

			//删除该用户的套接字
            q.exec(QString("UPDATE userinfo SET descriptor = '0' WHERE id = '%1'").arg(id));
...
void DoTcpServer::chatMessage_Slots(int socketDescriptor, QString userChatMessage)
{
	...
	{
		QTcpSocket* serverBroadcast = clientList.at(i);
        serverBroadcast->write(messageData);
	}
...
void DoTcpServer::userRegister_Slots(int socketDescriptor, QString userName_Register, QString userPassword_Register)
{
	...
	QSqlQuery q;
    q.exec("SELECT account FROM userinfo");
	while (q.next())
		for (int i = 100; i < 200; i++)
		{
			...
			{
				//存入数据库
                q.exec(QString("INSERT INTO userinfo(account,password,name,status) VALUES('%1','%2','%3',0)").arg(userAccount).arg(userPassword_Register).arg(userName_Register));
				...
				for (int i = 0; i < clientList.count(); i++)
				{
					...
					{
                        clientListTemp->write(checkData);
						return;
					}
				}
